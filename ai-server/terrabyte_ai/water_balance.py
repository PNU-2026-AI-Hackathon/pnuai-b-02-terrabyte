"""Soil water balance: the physics behind the synthetic training labels.

There is no measured irrigation data, so the labels are generated from a water
balance model rather than observed. ``evapotranspiration_pct_per_hour`` is
ported unchanged from ``edge/pi/tools/train_irrigation_rf.py`` so the edge
suppressor and the server recommender share one set of physical assumptions --
otherwise the edge could suppress a watering that the server just sized.

READ THIS BEFORE QUOTING ANY METRIC: labels produced here are a formula. A model
trained on them is recovering that formula, not learning about plants. See
