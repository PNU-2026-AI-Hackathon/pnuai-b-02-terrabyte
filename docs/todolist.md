...
### 0.4 ML 데이터 (2026-08-16 확정)

계약 전문은 [`design/ml_irrigation_contract.md`](design/ml_irrigation_contract.md)에 있다.

| # | 쟁점 | 결정 | 근거 |
|---|---|---|---|
| D23 | 관수 회귀 학습 데이터 | **물수지 기반 합성 데이터.** 엣지 RF의 증발산 모델을 이식해 라벨(mL)을 생성 | 데모까지 실측을 모을 수 없음. 엣지와 같은 물리 가정을 쓰면 두 모델의 판단이 모순되지 않음 |
| D24 | 합성 데이터 평가 점수 | **모델 성능으로 인용 금지.** "파이프라인 검증 완료, 실측 확보 진행 중"으로 표현 | 라벨이 수식이라 점수는 수식 복원 정확도일 뿐, 식물에 대해 아무것도 말해주지 않음 |
| D25 | 데이터셋 저장 | **CSV 커밋 금지. 생성기 + 시드만 커밋** | 엣지 RF에서 도구와 20MB CSV가 다른 브랜치에 흩어져 학습이 불가능했던 전례 |
| D26 | AI 서버 인증 | **기본 무인증(내부 네트워크 전용).** `AI_API_KEY` 설정 시에만 `X-Api-Key` 검증 활성화 | 프로덕션에서 호스트 포트를 열지 않으므로 기본 경로에 인증 비용을 넣을 이유가 없음 |
...
### P3-1. ML 데이터 계약 확정
브랜치: `docs/ml-contract`. 산출물: [`design/ml_irrigation_contract.md`](design/ml_irrigation_contract.md)

- [x] 입력 피처 확정 — 8개 (엣지 RF의 6개 + `substrate_volume_ml` + `crop_code`), `input_schema_version: 1`
- [x] 출력 확정 (권장 관수량 mL, 신뢰도) — `confidence`는 RF 트리 산포 기반 앙상블 합의도
- [x] REST 계약 문서화 (`POST /predict/irrigation`, `GET /health`)
- [x] 백엔드가 피처를 조달하는 방식 확정 — 최신 샘플 1건(`findLatest`), 시계열 윈도우 집계는 v1에서 쓰지 않음

### P3-2. 학습 데이터 확보
브랜치: `feature/ml-dataset`

- [x] 데이터 출처 확정 — **물수지 기반 합성 데이터.** 실측 확보 불가 (§8-2 해소)
- [x] 수집·라벨링 절차 문서화 — 라벨 공식 및 실측 전환 조건은 계약 문서 §4
- [x] 학습/검증 분할 및 평가 지표 정의 — 세션 단위 GroupShuffleSplit 70/15/15, MAE + 과다예측률
- [x] 데이터셋을 저장소에 커밋할지 외부 보관할지 결정 — **커밋하지 않는다.** 생성기 + 시드만 커밋
...
2. **룰 정의 저장 방식** — 코드 하드코딩(빠름) vs DB 설정 테이블(사용자 조정 가능). (P2-5)
3. **AI 서버 배포 위치** — 백엔드와 같은 인스턴스 vs 별도 인스턴스. (P3-3)
4. **Orange Pi 하드웨어 사양** — RF 추론 런타임(scikit-learn 등)을 올릴 여유가 있는지. (P3-5)
5. **데모 시연 방식** — 실제 하드웨어 실물 시연인지 화면 위주인지. P1-9와 P4-4의 범위가 달라진다.

> 결정이 나면 이 절에서 지우고 §0 Decision Log에 D번호를 붙여 추가한다.
> 2026-08-04까지 D6·D8·D9·D10·D11·D13이, 2026-08-16에 D23~D26이 이 방식으로 해소됐다.
