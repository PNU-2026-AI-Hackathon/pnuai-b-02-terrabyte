...
| 적용 범위 | **적청 LED 환경 제외** | 권장 모듈 제품이 미정이다. 함께 켜면 되먹임 루프가 생긴다 — [적용 범위](#적용-범위-적청-led-환경-제외) 참고 |
...
**이 설계는 적청(red/blue) 재배용 LED가 없는 환경만을 대상으로 한다.** 2026-08-23
결정이며, 권장할 LED 모듈의 제품이 아직 정해지지 않았기 때문이다.

### 왜 제외해야 하는가

lux는 사람 눈의 밝기 감도(광시감곡선)로 가중한 단위이고, 이 곡선은 555nm에서
최대이며 양 끝에서 급격히 떨어진다.
...
적청 LED는 눈이 거의 세지 못하는 파장에 출력이 몰려 있다. 광자는 쏟아지는데
lux는 미미하게 오르고, 식물의 광합성은 그 파장을 잘 쓴다. 그래서 PPFD/lux 계수가
백색 LED(0.0143)의 여러 배가 되며, `WHITE_GROW_LED`를 적용하면 광량을 크게
과소평가한다.

계수 하나로 보정될 문제가 아닐 수도 있다. TSL2591의 lux 계산식 자체가 광대역
광원을 가정하고 CH0(가시+적외)과 CH1(적외)의 비로 유도하므로, 협대역 광원에서는
그 가정이 무너진다.

### 이 제약이 위험한 이유

제품은 **조도 점수가 낮으면 재배용 LED 설치를 권장**한다. 상점에 `grow-light`
("식물 생장등 (LED)", `V12__create_product.sql`)가 '추천' 태그로 이미 등록되어
있다. 그 모듈이 적청 방식이라면 다음 루프가 성립한다.

```
조도 점수 낮음 → LED 설치 권장 → 사용자가 설치
                                   ↓
         점수 여전히 낮음  ←  센서가 PPFD를 크게 과소평가
                ↓
         LED 더 권장  →  (반복)
```

식물은 충분한 광량을 받는데 화면은 계속 부족하다고 말하고, 시스템은 조명을 더
팔려 든다. **광량 점수와 LED 권장을 함께 켜기 전에 반드시 해소해야 한다.**

### 해소 경로

모듈 제품이 정해지면 세 가지가 가능하다.

1. **모듈 전용 실측 계수** — 파는 제품이 정해져 있으므로 그 모듈의 PPFD/lux를 한
   번 실측해 상수로 둔다. 문헌 추정이 아니라 해당 제품에 대한 보정이라 정확하고,
   사용자는 "재배등 설치함"만 고르면 된다. **가장 현실적이다.**
2. **원시 채널값 전송** — lux 대신 TSL2591의 CH0/CH1 카운트를 보내고 채널 비율로
   광원을 판별해 환산한다. lux 공식의 왜곡을 우회하지만 작업량이 크다.
3. **PAR 센서 추가** — 가장 정확하지만 부품 비용이 든다.

1번으로 가면 "혼합" 문제도 자연스럽게 풀린다. LED는 기존 광원을 대체하는 것이
아니라 보태는 것이므로, **기본 광원 + 모듈 설치 여부**로 표현하고 모듈이 더하는
PPFD를 실측값으로 더하면 물리적으로도 맞다. 창가에 재배등을 더한 환경은 예외가
아니라 이 제품의 표준 시나리오이므로, 이 확장은 미루더라도 반드시 온다고 보아야
한다.
