...
| A2 | 유량계가 없다. 폴백 `0.98mL/s`는 설치 펌프 한 대·리그 한 곳의 510초 정상상태 1회 측정(500mL)이다 | 펌프·튜브 길이·헤드 높이 변경 시 재측정. 30초 이하 프라이밍 보정은 아직 필요 |
...
1. 설치 펌프의 10초·30초 실효 유량과 화분 배지 용적은? — 1회 관수 상한과 일일 예산 산정에 직접 들어간다.
...
app:
  irrigation:
    ...
    dose-min-ml: 20
    dose-max-ml: 200
    default-flow-ml-per-s: 0.98     # 한 펌프·한 리그, 500mL/510초 정상상태 1회 실측
    command-ttl: PT2M
    implausible-jump-pct: 30
```

이 값은 설정이 없을 때만 쓰는 폴백이며 보편 상수가 아니다. 펌프·튜브 길이·헤드 높이가 바뀌면
다시 측정한다. 캠페인 구동은 30초 이하라 프라이밍으로 실효 유량이 더 낮지만, 10초 × 5회와
30초 반복 보정은 아직 하지 않았다. 한 번의 510초 측정뿐이므로 표준편차도 없다.

> **운영 경고:** 200mL는 예전 `8.0mL/s` 가정에서 25초였지만 지금은 204,082ms(약 204초)다.
> 이전 30초 G1 상한을 유지하는 노드는 큰 용량을 모두 잘라 내고 `stop:"max_runtime"`을
> 보고한다. 이 변경은 폴백만 교체하며 펌웨어 인터록은 변경하지 않는다.
...
- **나중:** 작물별 예산 프로파일(현재는 전역 상수), 예산 회복 곡선(현재는 24h 롤링 합), 10초·30초 단기 유량 보정
...
// dn/command
{
  ...
  "actuator": "pump",
  "action": "dose",
  "params": { "volume_ml": 120, "max_runtime_ms": 122449 },
